...
void baum_welch(double* PI, double* A, double* B, int* O, double* FW, double* BW, double* C, int N, int M, int T, int n_iter) {
    ...
    for(int it = 0; it < n_iter; it++) {
        ...
        for(int i = 0; i < N; i+=8) {
            ...
            double pi4 = FW[i+4] * BW[i+4] * scales0;
            double pi5 = FW[i+5] * BW[i+5] * scales0;
            double pi6 = FW[i+6] * BW[i+6] * scales0;
            double pi7 = FW[i+7] * BW[i+7] * scales0;
            ...
            for(int t = 1; t < T-1; t+=1) {
                ...
                double toaddi4 = FW[(i+4) + t*N] * BW[(i+4) + t*N] * scales[t];
                double toaddi5 = FW[(i+5) + t*N] * BW[(i+5) + t*N] * scales[t];
                double toaddi6 = FW[(i+6) + t*N] * BW[(i+6) + t*N] * scales[t];
                double toaddi7 = FW[(i+7) + t*N] * BW[(i+7) + t*N] * scales[t];
                ...
                sum_os[(i+4) + O[t]*N] += toaddi4;
                sum_os[(i+5) + O[t]*N] += toaddi5;
                sum_os[(i+6) + O[t]*N] += toaddi6;
                sum_os[(i+7) + O[t]*N] += toaddi7;
            }
            denomi0 += pi0;
            denomi1 += pi1;
            denomi2 += pi2;
            ...
            double lastaddi4 = FW[(i+4) + (T-1)*N] * BW[(i+4) + (T-1)*N] * scalest1;
            double lastaddi5 = FW[(i+5) + (T-1)*N] * BW[(i+5) + (T-1)*N] * scalest1;
            double lastaddi6 = FW[(i+6) + (T-1)*N] * BW[(i+6) + (T-1)*N] * scalest1;
            double lastaddi7 = FW[(i+7) + (T-1)*N] * BW[(i+7) + (T-1)*N] * scalest1;

            denoms[i+0] = denomi0 + lastaddi0;
            denoms[i+1] = denomi1 + lastaddi1;
            denoms[i+2] = denomi2 + lastaddi2;
            denoms[i+3] = denomi3 + lastaddi3;
            denoms[i+4] = denomi4 + lastaddi4;
            denoms[i+5] = denomi5 + lastaddi5;
            denoms[i+6] = denomi6 + lastaddi6;
            denoms[i+7] = denomi7 + lastaddi7;

            sum_os[(i+0) + ot1*N] += lastaddi0;
            sum_os[(i+1) + ot1*N] += lastaddi1;
            sum_os[(i+2) + ot1*N] += lastaddi2;
            sum_os[(i+3) + ot1*N] += lastaddi3;
            sum_os[(i+4) + ot1*N] += lastaddi4;
            sum_os[(i+5) + ot1*N] += lastaddi5;
            sum_os[(i+6) + ot1*N] += lastaddi6;
            sum_os[(i+7) + ot1*N] += lastaddi7;
            ...
            for(int j = 0; j < N; j += 4) {
                ...
                for(int t = 0; t < T-1; t+=1) {
                    ...
                    numi4j0 += FW[(i+4) + (t  )*N] * B[(j+0) + O[t+1]*N] * BW[(j+0) + (t+1)*N];
                    numi5j0 += FW[(i+5) + (t  )*N] * B[(j+0) + O[t+1]*N] * BW[(j+0) + (t+1)*N];
                    numi6j0 += FW[(i+6) + (t  )*N] * B[(j+0) + O[t+1]*N] * BW[(j+0) + (t+1)*N];
                    numi7j0 += FW[(i+7) + (t  )*N] * B[(j+0) + O[t+1]*N] * BW[(j+0) + (t+1)*N];
                    ...
                    numi4j1 += FW[(i+4) + (t  )*N] * B[(j+1) + O[t+1]*N] * BW[(j+1) + (t+1)*N];
                    numi5j1 += FW[(i+5) + (t  )*N] * B[(j+1) + O[t+1]*N] * BW[(j+1) + (t+1)*N];
                    numi6j1 += FW[(i+6) + (t  )*N] * B[(j+1) + O[t+1]*N] * BW[(j+1) + (t+1)*N];
                    numi7j1 += FW[(i+7) + (t  )*N] * B[(j+1) + O[t+1]*N] * BW[(j+1) + (t+1)*N];
                    ...
                    numi4j2 += FW[(i+4) + (t  )*N] * B[(j+2) + O[t+1]*N] * BW[(j+2) + (t+1)*N];
                    numi5j2 += FW[(i+5) + (t  )*N] * B[(j+2) + O[t+1]*N] * BW[(j+2) + (t+1)*N];
                    numi6j2 += FW[(i+6) + (t  )*N] * B[(j+2) + O[t+1]*N] * BW[(j+2) + (t+1)*N];
                    numi7j2 += FW[(i+7) + (t  )*N] * B[(j+2) + O[t+1]*N] * BW[(j+2) + (t+1)*N];
                    ...
                    numi4j3 += FW[(i+4) + (t  )*N] * B[(j+3) + O[t+1]*N] * BW[(j+3) + (t+1)*N];
                    numi5j3 += FW[(i+5) + (t  )*N] * B[(j+3) + O[t+1]*N] * BW[(j+3) + (t+1)*N];
                    numi6j3 += FW[(i+6) + (t  )*N] * B[(j+3) + O[t+1]*N] * BW[(j+3) + (t+1)*N];
                    numi7j3 += FW[(i+7) + (t  )*N] * B[(j+3) + O[t+1]*N] * BW[(j+3) + (t+1)*N];
                }
                ...
                A[(i+4)*N + (j+0)] = numi4j0*A[(i+4)*N + (j+0)]/denomi4;
                A[(i+5)*N + (j+0)] = numi5j0*A[(i+5)*N + (j+0)]/denomi5;
                A[(i+6)*N + (j+0)] = numi6j0*A[(i+6)*N + (j+0)]/denomi6;
                A[(i+7)*N + (j+0)] = numi7j0*A[(i+7)*N + (j+0)]/denomi7;
                ...
                A[(i+4)*N + (j+1)] = numi4j1*A[(i+4)*N + (j+1)]/denomi4;
                A[(i+5)*N + (j+1)] = numi5j1*A[(i+5)*N + (j+1)]/denomi5;
                A[(i+6)*N + (j+1)] = numi6j1*A[(i+6)*N + (j+1)]/denomi6;
                A[(i+7)*N + (j+1)] = numi7j1*A[(i+7)*N + (j+1)]/denomi7;
                ...
                A[(i+4)*N + (j+2)] = numi4j2*A[(i+4)*N + (j+2)]/denomi4;
                A[(i+5)*N + (j+2)] = numi5j2*A[(i+5)*N + (j+2)]/denomi5;
                A[(i+6)*N + (j+2)] = numi6j2*A[(i+6)*N + (j+2)]/denomi6;
                A[(i+7)*N + (j+2)] = numi7j2*A[(i+7)*N + (j+2)]/denomi7;
                ...
                A[(i+4)*N + (j+3)] = numi4j3*A[(i+4)*N + (j+3)]/denomi4;
                A[(i+5)*N + (j+3)] = numi5j3*A[(i+5)*N + (j+3)]/denomi5;
                A[(i+6)*N + (j+3)] = numi6j3*A[(i+6)*N + (j+3)]/denomi6;
                A[(i+7)*N + (j+3)] = numi7j3*A[(i+7)*N + (j+3)]/denomi7;
            }
        }
