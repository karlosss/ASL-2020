...
void baum_welch(double* PI, double* A, double* B, int* O, double* FW, double* BW, double* C, int N, int M, int T, int n_iter) {
    ...
    for(int it = 0; it < n_iter; it++) {
        ...
        for(int i = 0; i < N; i+=8) {
            ...
            _mm256_storeu_pd(denoms+i, _mm256_div_pd(_mm256_set1_pd(1.0), _mm256_fmadd_pd(vec_lastadd0, vec_scalest1, vec_denomi0)));
            _mm256_storeu_pd(denoms+i+4, _mm256_div_pd(_mm256_set1_pd(1.0), _mm256_fmadd_pd(vec_lastadd1, vec_scalest1, vec_denomi1)));

            vec_denomi0 = _mm256_div_pd(_mm256_set1_pd(1.0), vec_denomi0);
            vec_denomi1 = _mm256_div_pd(_mm256_set1_pd(1.0), vec_denomi1);
            ...
            for(int j = 0; j < N; j += 4) {
                ...
                _mm256_storeu_pd(A+(i+2)*N+j, _mm256_mul_pd(_mm256_mul_pd(numi2, a2), bc_denomi2));
                _mm256_storeu_pd(A+(i+3)*N+j, _mm256_mul_pd(_mm256_mul_pd(numi3, a3), bc_denomi3));
                _mm256_storeu_pd(A+(i+4)*N+j, _mm256_mul_pd(_mm256_mul_pd(numi4, a4), bc_denomi4));
                _mm256_storeu_pd(A+(i+5)*N+j, _mm256_mul_pd(_mm256_mul_pd(numi5, a5), bc_denomi5));
                _mm256_storeu_pd(A+(i+6)*N+j, _mm256_mul_pd(_mm256_mul_pd(numi6, a6), bc_denomi6));
                _mm256_storeu_pd(A+(i+7)*N+j, _mm256_mul_pd(_mm256_mul_pd(numi7, a7), bc_denomi7));
            }
        }
        ...
        for(int o = 0; o < M; o += 1){
            for(int i = 0; i < N; i+=8) {
                ...
                __m256d res0 = _mm256_mul_pd(sum_os_io0, denoms_i0);
                __m256d res1 = _mm256_mul_pd(sum_os_io1, denoms_i1);
